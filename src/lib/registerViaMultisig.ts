 ...
 *
 * Known constraints:
 *  1. MIDEN_GUARDIAN_ENDPOINT (constants.ts) must point at the SAME guardian
 *     operator the wallet registered the account with, or `load()` fails. There is
 *     no on-chain way to discover it (the account stores only a guardian commitment,
 *     not a URL) — the wallet doesn't expose it either, so we pin one operator's
 *     endpoint alongside the other network addresses.
...
import { MIDEN_GUARDIAN_ENDPOINT } from '@/shared/constants';
import { generateRandomSerialNumber } from './midenClient';

// The guardian/PSM operator the wallet's accounts are registered with. Must match
// the wallet's guardian, or MultisigClient.load() fails.
const GUARDIAN_ENDPOINT = MIDEN_GUARDIAN_ENDPOINT;
